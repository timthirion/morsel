...
pub fn save<P: AsRef<Path>, I: MeshIndex>(mesh: &HalfEdgeMesh<I>, path: P) -> Result<()> {
    ...
    // `double`, not `float`: positions are `f64` in memory and are written at
    // full precision below, so declaring 32-bit would tell readers to throw half
    // of every coordinate away. `double` is standard PLY and this module's reader
